...
The loop has ten fixed checkpoints around a single core. You **feed** them, you
...
A single central node, `_core`, holds the loop's state (the Karpathy "LLM Wiki" pattern as
one note). Every checkpoint links it and it links every checkpoint, so the graph is a dense,
orphan-free web around one core. Open `_core` first. It carries, as sections:

- **the loop** — the ordered checkpoint list and acceptance snapshot.
- **## Schema** — the conventions that govern how every checkpoint behaves.
- **## Sources** — the raw-layer index over the immutable `topics/<slug>/` artifacts.
- **## Log** — the append-only chronological record. Closeout (08) prepends one line per pass.
...
1. **Resolve the vault.** Pick or confirm a target directory for this goal. If it has no
   ...
   command-center index, the single hub), then its `07 - Hot.md`
...
3. **Hot, then Closeout.** Overwrite the `07 - Hot.md` live-state block (never append it).
   Write the five-part closeout in `08 - Closeout.md` and prepend a one-line entry to the
   **Log** section of `_core`.
...
- **Prune is archive-only.** Never delete directly: write a dry-run manifest, get explicit
  ...
  `_core`, `goaloop/`, `scripts/`, or `Loop.canvas`.
...
- `${CLAUDE_SKILL_DIR}/template/checkpoints/00..09.md` are the checkpoint bodies and
  `template/core.md` is the core body (index + schema + sources + log sections); the generator
  owns titles, frontmatter, and wiring.
